...
class Shape:
    ...
    def move_down(self):
        ...
        self.boundingbox = [
            Point(y=coord.y + 1, x=coord.x) for coord in self.boundingbox
        ]
    ...
    def move_left(self, leftlimit):
        ...
        self.boundingbox = [
            Point(y=coord.y, x=coord.x - 1) for coord in self.boundingbox
        ]
    ...
    def rotate_clockwise(self, rightlimit, leftlimit):
        ...
        shape_relative = [
            Point(y=coord.y - bb_y, x=coord.x - bb_x) for coord in self.shape
        ]
        boundingbox_relative = [
            Point(y=coord.y - bb_y, x=coord.x - bb_x)
            for coord in self.boundingbox
        ]
        self.shape = [
            rotate_point_clockwise(coord, origin) for coord in shape_relative
        ]
        self.boundingbox = [
            rotate_point_clockwise(coord, origin)
    ...
    def modify_coordinates(self, diff):
        self.shape = [
            Point(y=coord.y, x=coord.x + diff) for coord in self.shape
        ]
        self.boundingbox = [
            Point(y=coord.y, x=coord.x + diff) for coord in self.boundingbox
        ]
